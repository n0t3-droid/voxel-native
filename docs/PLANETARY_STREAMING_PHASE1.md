...
The latest available real-engine QA evidence before this phase measured:

| QA run | Effective render distance | Resident full chunks |
| --- | ---: | ---: |
| `run_1786302248` | 16 chunks (256 m nominal radius) | 7,204 |
| `run_1786302494` | 23 chunks (368 m nominal radius) | 8,193 |

Those runs predate the now-implemented dense-near hard ceiling. The current
streamer caps full chunks at 2,400, the interaction radius at 16 chunks, terrain
tasks at 96, and mesh tasks at 64. The historical values remain useful as the
measured failure baseline; they are not the current allowed residency budget.
...
At a 30.72 km radius, the same representation would contain roughly 11.58
million X/Z columns and is therefore not a viable far-field representation.
...
The deterministic inputs are `(seed, world profile, scenery quality)`, signed
64-bit camera X/Z, the bounded interaction radius, and the automatic pressure
signal. The output is exactly one complete finest fallback grid plus five
render-only annuli extending to 30.72 km.
...
finest-grid stencil removes each 32 m parent cell only after all four underlying
16 m chunk columns have current voxel data and settled or already-visible mesh
...
- Level spacing is `32, 64, 128, 256, 512, 1024` metres.
- Their outer extents are `0.96, 1.92, 3.84, 7.68, 15.36, 30.72` km.
- The finest level is a complete fallback until the near streamer proves
  individual current-request columns. A 3,600-bit stencil hides a 32 m parent
  cell only when every one of its four 16 m columns is covered. Missing data,
...
vertex maps by Euclidean absolute world coordinates to a fixed 128 m material
...
parity. A representative can be 64 m away along either axis, so transitions
...
profile. The pre-change distribution above is the direct comparison baseline.
Nine post-change cold six-ring runs measured:
...
| `BridgeV2` | 5.364 ms | 5.944 ms | fixed 128 m categorical cells; default |
...
The steady 32-step diagonal route produced 94 incremental ring jobs per run.
...
the 30.72-km extent. Teleport fallback is intentionally slower than a strip,
but fixed: it allocates no path history and cannot enqueue more than one job.
...
35,000 / 150,000 limits. At 30.72 km the old full-chunk circle would contain
11,581,133 horizontal chunk columns (up to 92,649,064 configured vertical
slots), while the chosen representation remains six render entities.

Forty-three focused tests pass (four manual benchmarks ignored). They pin the
topology and byte budgets, cold/incremental byte equality, exact axial and
diagonal strip counts, deterministic replay, signed/extreme coordinates,
...
### Real-engine evidence after the coverage stencil

The following captures predate the final BridgeV2 material path. They remain
valid evidence for geometry, coverage, travel, and fixed budgets, but they are
not visual acceptance evidence for BridgeV2. A same-seed, same-hour Legacy vs
BridgeV2 comparison plus Natural and Astral anchor/LOD-transition routes is
still pending.

All evidence below used seed 12345, Lush scenery, 1280 x 720, hour 15.65 and the
Debug executable. FPS is therefore a route-specific observation, not a release
or hardware promise.

| Run | Route | Result |
| --- | --- | --- |
| `run_1786329313` | Astral 20 s hero inspection | 176 ready Near columns, 37 hidden parent cells, 6 far entities, 28,086 vertices, 117,738 indices, 1,819,080 mesh bytes, 0 budget rejects; old pink/grey parent sheets across the voxel foreground absent |
| `run_1786329471` | Natural 8 km / 12 s stress | 8,000 m displacement, 6 entities, 28,086 vertices, 117,954 indices, 1,819,944 mesh bytes, 0 budget rejects, 162.87 observed average FPS |
| `run_1786329490` | Astral 8 km / 12 s stress | 8,000 m displacement, 6 entities, 28,086 vertices, 117,960 indices, 1,819,968 mesh bytes, 0 budget rejects, 156.07 observed average FPS |

The hero comparison proves the irregular cutout removes the large coarse sheets
that previously crossed settled voxel relief. The stress routes prove the full
parent safely remains when the near streamer cannot follow a roughly 667 m/s
camera: no inner skirt wall, sky hole, stale world, or horizon contraction was
seen in the captured sequence. Time-based coverage stabilisation reduced the
first irregular-stencil hero run from 226 completed / 64 stale builds to 118 / 15
without changing final coverage.

The same images also reject a stronger art claim: the 32 m surface is visibly
pastel and smooth beside detailed voxel mountains. This is now the leading
midfield-quality gap, not a correctness gap. Before broad rollout, continue:

1. Inspect all six compass directions and a top-down view at 0, 5, and 25 km.
2. Fly across multiple 32/64/128 m anchor transitions and look specifically for
...
Known Phase-1 limits are intentional: the far field is a terrain surface, not a
replacement for mid-field bricks, sparse edit summaries, distant structures,
water, vegetation impostors, or simulation bubbles. Those systems should layer
